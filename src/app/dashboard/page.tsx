...
const mockComplaints = [
  { id: 'CMP-1042', title: 'Water leakage in Hostel B, Room 302', category: 'Water', status: 'pending', priority: 'high', date: '2026-07-30' },
  { id: 'CMP-1041', title: 'Broken projector in Academic Block A, Room 201', category: 'Classroom', status: 'in_progress', priority: 'medium', date: '2026-07-29' },
  { id: 'CMP-1038', title: 'Internet outage in Computer Center', category: 'Internet', status: 'resolved', priority: 'critical', date: '2026-07-25' },
  { id: 'CMP-1035', title: 'Mess food quality complaint', category: 'Mess', status: 'escalated', priority: 'high', date: '2026-07-20' },
  { id: 'CMP-1029', title: 'Streetlight not working near Library', category: 'Security', status: 'resolved', priority: 'low', date: '2026-07-15' },
]
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div className="bg-white p-5 rounded-xl border border-[#EAEAEA] flex items-start justify-between">
          <div>
            ...
            <p className="text-2xl font-semibold text-[#111111]">12</p>
          </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-[#EAEAEA] flex items-start justify-between">
          <div>
            ...
            <p className="text-2xl font-semibold text-[#111111]">3</p>
          </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-[#EAEAEA] flex items-start justify-between">
          <div>
            ...
            <p className="text-2xl font-semibold text-[#111111]">8</p>
          </div>
        ...
        <div className="bg-white p-5 rounded-xl border border-[#EAEAEA] flex items-start justify-between">
          <div>
            ...
            <p className="text-2xl font-semibold text-[#111111]">1</p>
          </div>
            ...
            <tbody className="divide-y divide-[#EAEAEA]">
              {mockComplaints.map((comp) => (
                <tr key={comp.id} className="hover:bg-[#F7F6F3] transition-colors group cursor-pointer">
                  ...
                  <td className="px-6 py-4">
                    <span className="inline-flex items-center px-2 py-1 rounded border border-[#EAEAEA] bg-white text-xs font-medium text-[#787774]">
                      {comp.category}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${STATUS_STYLES[comp.status]}`}>
                      {comp.status.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`inline-flex items-center px-2 py-1 rounded text-xs capitalize ${PRIORITY_STYLES[comp.priority]}`}>
                      {comp.priority}
                    </span>
                  </td>
